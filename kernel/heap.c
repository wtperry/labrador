    ...
    while (block) {
        if (!block->used && block->size >= size) {
            ...
            if (next_free == block) {
                ...
                if (next_free->used) {
                    //FIXME: current algorithm requires that we leave at least
                    //       one free block in the allocator
...
void dump_heap() {
    ...
    while (block) {
        log_printf(LOG_DEBUG, "addr: %.16lx    prev: %.16lx    next: %.16lx    size: %lx    used: %d", block, block->prev, block->next, block->size, block->used);
        block = block->next;
    }
}